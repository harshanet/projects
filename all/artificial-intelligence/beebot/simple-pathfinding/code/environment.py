...
class Environment:
    ...
    def get_possible_actions(self, state):
        ...
        for action in BEE_ACTIONS:
            success, _, _ = self.perform_action(state, action)
            if success:
                actions.append(action)
        return actions
    
    def get_action_cost(self, state, action):
        _, cost, _ = self.perform_action(state, action)
        return cost
    
    def get_child_state(self, action, state):
        _, _, child_state = self.perform_action(state, action)
        return child_state
